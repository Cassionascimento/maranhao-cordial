...
(() => {
  ...
  const materialLabel = v => ({PRESENTATION:'Apresentação executiva',IMAGE:'Imagem',EXECUTIVE_DECK:'Apresentação executiva',LABEL_CONCEPT:'Conceito de rótulo',SOCIAL_CREATIVE:'Material de campanha',PACKAGING_CONCEPT:'Conceito de embalagem',INFOGRAPHIC:'Infográfico',REPORT:'Relatório',CHART:'Gráfico',generated:'Gerado',draft:'Rascunho',approved:'Aprovado',rejected:'Rejeitado',gerado:'Gerado',aprovado:'Aprovado',rejeitado:'Rejeitado',historico:'Arquivado'}[v] || v || 'Não informado');
  const technicalDetails = data => `<details><summary>Dados técnicos</summary><pre style="white-space:pre-wrap;overflow-wrap:anywhere">${esc(JSON.stringify(data,null,2))}</pre></details>`;
  function materialDetails(data) {
    const versions=arr(data.linhagem), events=arr(data.auditoria);
    return `<section class="mic-sub"><h4>Versões do material</h4>${versions.map(a=>`<article><h5>${esc(materialLabel(a.artifact_type))} · Versão ${esc(a.version)}</h5><p>Estado: ${esc(materialLabel(a.status))}</p><p>${esc(a.metadata?.titulo || a.metadata?.title || a.metadata?.briefing || 'Descrição não registrada.')}</p><p>Criado em: ${esc(a.created_at || 'Não registrado')}</p><p>Origem: ${esc(a.source_type || 'Não informada')}</p><button data-creative-action="open" data-artifact="${esc(a.id)}">Abrir esta versão</button><button data-creative-action="download" data-artifact="${esc(a.id)}">Baixar esta versão</button></article>`).join('') || empty('Nenhuma versão disponível.')}<h4>Histórico de decisões</h4>${events.map(e=>`<p>${esc(e.criado_em || 'Data não registrada')} · ${esc(e.ator || 'Responsável não registrado')} · ${esc(materialLabel(e.estado_novo))}${e.motivo ? ' — '+esc(e.motivo) : ''}</p>`).join('') || empty('Nenhuma decisão registrada.')}${technicalDetails(data)}</section>`;
  }
  function meetingDetails(data) {
    return `<section class="mic-sub"><h4>${esc(data.titulo || 'Ata do Conselho')}</h4><p>${esc(data.narrativa || 'Narrativa não disponível.')}</p><h5>Decisões</h5><ul>${arr(data.decisoes).map(v=>`<li>${esc(v)}</li>`).join('') || '<li>Nenhuma decisão registrada.</li>'}</ul><h5>Próximos passos</h5><ul>${arr(data.proximos_passos).map(v=>`<li>${esc(v)}</li>`).join('') || '<li>Nenhum próximo passo registrado.</li>'}</ul>${technicalDetails(data)}</section>`;
  }
  async function loadCreative() {
    const reads=[['/api/admin/mi/artefatos','mic-creative-artifacts',b=>{
      $('mic-creative-artifacts').innerHTML=arr(b.artefatos).map(a=>`<article class="mic-sub"><span>${esc(materialLabel(a.artifact_type))}</span><h4>Versão ${esc(a.version)} · ${esc(materialLabel(a.status))}</h4><small>${esc(a.metadata?.production_status||a.metadata?.confidence||'Proveniência não informada')}</small>
      <div>${['open','download','detail','approve','reject',...(['image/png','image/jpeg','image/webp'].includes(a.mime_type)?['refine','social']:[])].map(action=>`<button data-creative-action="${action}" data-artifact="${esc(a.id)}">${({open:'Abrir',download:'Baixar',detail:'Detalhes e histórico',approve:'Aprovar',reject:'Rejeitar',refine:'Nova versão',social:'Feed / story / mockup'})[action]}</button>`).join('')}</div></article>`).join('')||empty('Nenhum artefato registrado.');
  ...
  async function creativeClick(e) {
    ...
    try {
      ...
      else if(action==='ata'){result=await get('/api/admin/mi/conselho/'+encodeURIComponent(record)+'/ata');$('mic-creative-detail').innerHTML=meetingDetails(result);}
      ...
      else if(action==='detail'){result=await get(artifactPath(id)+'/linhagem');const audit=await get(artifactPath(id)+'/auditoria');$('mic-creative-detail').innerHTML=materialDetails({...result,...audit});}
